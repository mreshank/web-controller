...
import { collectOrb, getSlowFactor, rewardBoostPad } from "@/game/game-rules";
...
export function PlayerRover({ slotIndex, color, spawn }: PlayerRoverProps) {
  ...
  useEffect(() => {
    const onHit = (e: Event) => {
      const d = (e as CustomEvent<{ slot: number }>).detail;
      if (d?.slot === slotIndex) hitFlashRef.current = performance.now() + 500;
    };
    window.addEventListener("game-hazard", onHit);
    return () => window.removeEventListener("game-hazard", onHit);
  }, [slotIndex]);
  ...
  useFrame((_, delta) => {
    ...
    const slow = getSlowFactor(slotIndex);

    const baseSpeed = (12 + s.triggers.right * 18) * boostRef.current * slow;
    const yaw = cameraRig.yawWorld;
    const ax = s.leftStick.x;
    const ay = s.leftStick.y;

    let mx =
      (Math.sin(yaw) * ay + Math.cos(yaw) * ax) * baseSpeed * dt + dashVelRef.current.x * dt;
    let mz =
      (Math.cos(yaw) * ay - Math.sin(yaw) * ax) * baseSpeed * dt + dashVelRef.current.z * dt;
    ...
    g.rotation.y = THREE.MathUtils.lerp(g.rotation.y, facingRef.current, 1 - Math.exp(-14 * dt));

    dashVelRef.current.x *= Math.exp(-12 * dt);
    dashVelRef.current.z *= Math.exp(-12 * dt);

    const now = performance.now();
    if (s.buttons[0] && now - lastDashRef.current > DASH_COOLDOWN_MS && slow >= 0.9) {
    ...
    boostRef.current = THREE.MathUtils.lerp(boostRef.current, 1, 1 - Math.exp(-4 * dt));
    ...
    for (let i = 0; i < boostPads.length; i++) {
      ...
      if (dx * dx + dz * dz < pad.radius * pad.radius) {
        ...
        if (now - last > 2200) {
          ...
          boostRef.current = 1.9;
          gameState.boostPadsHit += 1;
          rewardBoostPad(slotIndex);
        }
      }
    }

    if (bodyMatRef.current) {
      const hit = now < hitFlashRef.current;
      bodyMatRef.current.emissiveIntensity = hit
        ? 2.5
        : 0.4 + s.triggers.right * 1.1 + (boostRef.current > 1.2 ? 1.3 : 0);
      bodyMatRef.current.color.set(hit ? "#fb7185" : color);
      bodyMatRef.current.emissive.set(hit ? "#fb7185" : color);
    }
    ...
    if (orbId !== null) {
      const points = collectOrb(slotIndex, orbId);
